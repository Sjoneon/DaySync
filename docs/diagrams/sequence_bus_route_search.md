```mermaid
'''mermaid sequenceDiagram actor User as 사용자 participant RF as RouteFragment participant API as DaySyncApiService participant Server as FastAPI Server participant TAGO as 국토교통부 API participant TMAP as TMAP API participant DB as MySQL Database
